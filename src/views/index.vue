<template>
  <div>
    ...
    <v-row v-if="now_data">
      <cardItem :id="n.id" :name="n.name" :img="n.image" :is_bofang="false" :is_new="true" :rank="n.rating" v-for="n in now_data" :key="n.id" />
    </v-row>

    <div class="text-h5 mt-6 subtitle">热门电影</div>
    <v-row v-if="movie_data">
      <cardItem :id="n.id" :name="n.title" :img="n.cover" :is_bofang="n.playable" :is_new="n.is_new" :rank="n.rate" v-for="n in movie_data" :key="n.id" />
    </v-row>

    <div class="text-h5 mt-6 subtitle">热门电视剧</div>
    <v-row v-if="movie_data">
      <cardItem :id="n.id" :name="n.title" :img="n.cover" :is_bofang="n.playable" :is_new="n.is_new" :rank="n.rate" v-for="n in tv_data" :key="n.id" />
    </v-row>
  </div>
</template>
<script>
